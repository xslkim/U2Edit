...
- **v1.3 (2026-04-15)**：
  ...
  - 「测试范围说明」补充：**QA 测试暂缓** 的任务达到 `dev_done` 即视为满足后续任务的前置依赖（等同 `passed`，见 [status.md](status.md)）
- **v1.2 (2026-04-15)**：
...
- **TypeScript `tsc --noEmit` 门禁仅适用于应用主仓库**（Tauri/Vue 部分，即 `src/`、`src-tauri/` 之外的 TS 代码）。`poc/unity/`、`poc/unreal/` 等独立引擎工程**不在 tsc 门禁范围内**，按各自工程的语言规范（C# 编译 / Python lint）验证
- 应用代码无 any 类型（除非加注释说明）
...
- **当前阶段 Unity / Unreal 导出链路暂不进行 QA 验收**，涉及任务：T0.4、T0.5、T3.1、T3.2、T3.3、T4.1、T4.2。这些任务的代码可正常开发并提交 `dev_done`，但 QA Agent 不进入 `testing`，状态停留在 `dev_done` 直到人工解锁。**依赖解锁**：上述任务一旦 `dev_done`，即视为满足依赖它们的后续任务之前置条件（**`dev_done` 等同 `passed`**），例如 T5.3 可在 M4 相关任务均为 `dev_done` 后启动开发
- **非功能性指标的判定门槛**：
  - 安装包体积 `< 15MB`（T0.1 / T5.3 用例）属**信息性指标**，超标不算 Dev 缺陷，记录到 T5.3 的"打包优化"备注里追踪；只有 `< 50MB` 才作为硬失败门槛
  - FPS、内存等性能指标见 T5.2，按里程碑级回归验证，单任务不卡门禁
...
- **测试用例**：
  ...
  4. 执行 `pnpm tauri build`，在 `src-tauri/target/release/bundle/` 下生成 `.msi` 或 `.exe` 安装包；体积 `< 15MB` 为信息性指标（见全局"测试范围说明"），硬门槛为 `< 50MB`
...
- **交付物**：
  ...
  - **浮层关闭统一规则**（产品口径，三选一明确）：
    - **确认按钮** = 提交当前值，关闭，入栈 1 次
    - **点击外部**（浮层之外的任意区域，包括画布、其他面板）= 等同"确认"，提交并入栈 1 次
    - **Esc** 或 **取消按钮** = 放弃，节点颜色回到打开前值，不入栈
  - 拖拽过程实时预览（画布实时更新），仅在"提交"路径关闭时入栈一次
  - `src/panels/AssetPicker.vue`：缩略图预览按钮 → 点击弹出 assets 列表（缩略图网格）+ 搜索框 + 清除按钮（置 null）。关闭规则同上：点缩略图 = 提交；点外部 = 提交当前选择；Esc = 放弃
...
- **测试用例**：
  ...
  4. Esc 关闭浮层时节点颜色回到打开前值，且 Ctrl+Z 不会撤销出"打开浮层"前的状态（即未入栈）
  5. 拖选后点击浮层之外的画布区域，行为等同"确认"：颜色保留，Ctrl+Z 仅撤销一次
  6. 点击 assetId 字段打开选择器，列出所有 assets 缩略图
  7. 搜索框输入 "btn"，列表过滤；点某个缩略图后关闭浮层，节点 assetId 更新
  8. 点"清除"按钮，assetId 设为 null，画布对应节点显示占位
